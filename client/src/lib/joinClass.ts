 ...
 *
 * One place, so every Join button behaves the same. There are two of them on
 * the student side — the watch page and the booking modal — and a copy each
 * would drift the first time an error case changed.
 *
...
export async function redirectToClass(liveClassId: string, opts: { visible?: boolean } = {}): Promise<never | void> {
  const res = await api.post<{ success: true; data: HandoffResponse }>(
    `/live-classes/${liveClassId}/handoff`,
    opts,
  )
